...
if ($pdo && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (($_POST['action'] ?? '') === 'delete') {
        $id = (int) ($_POST['id'] ?? 0);
        if ($id > 0) {
            db_execute($pdo, 'DELETE FROM pages WHERE id = :id AND page_group = :grp', [':id' => $id, ':grp' => 'why_choose_us']);
            cms_invalidate_page_cache();
            admin_flash('success', 'Why page deleted.');
        }
...
<div class="filter-bar">
  <form class="filter-row" method="get">
    <div class="filter-group">
      ...
      <input class="form-control" name="q" value="<?= e($q) ?>" placeholder="Search why pages...">
    </div>
    <div class="filter-group">
    ...
    <div class="filter-group">
      ...
      <div class="d-flex gap-2">
        ...
        <a class="btn btn-secondary-modern" href="why-page-edit.php">Add Why Page</a>
      </div>
    </div>
  </form>
</div>
...
<div class="widget-card">
  <div class="widget-header">
    <h5 class="widget-title">Why Choose Us Pages (<?= count($rows) ?>)</h5>
      ...
      <tbody>
        <?php foreach($rows as $r): ?>
          <tr>
            ...
            <td>
              <div class="d-flex gap-2">
                <a class="btn btn-outline-primary btn-sm" href="why-page-edit.php?id=<?= (int)$r['id'] ?>" title="Edit">
        ...
        <?php if (!$rows): ?>
          <tr>
            <td colspan="5" class="text-center text-muted py-4">No why pages found.</td>
          </tr>
        <?php endif; ?>
      </tbody>
    </table>
