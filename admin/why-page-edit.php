...
if ($pdo && $id > 0) {
  $row = db_fetch_one($pdo, 'SELECT p.*, pm.meta_title, pm.meta_description, pm.meta_keywords, pm.canonical_url FROM pages p LEFT JOIN page_meta pm ON pm.page_id = p.id WHERE p.id = :id AND p.page_group = :grp LIMIT 1', [':id'=>$id, ':grp'=>'why_choose_us']);
  if ($row) {
    $pageData = array_merge($pageData, $row);
    $previousSlug = (string) ($row['slug'] ?? '');
    $accordionRows = db_fetch_all($pdo, 'SELECT * FROM why_page_accordions WHERE page_id = :pid ORDER BY sort_order ASC, id ASC', [':pid' => $id]);
...
if ($pdo && $_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($id > 0) {
    db_execute($pdo, 'UPDATE pages SET title=:t, slug=:s, status=:st, content=:c, page_group=:grp, template_key=:tpl WHERE id=:id', [
      ':t'=>$titleIn, ':s'=>$slug, ':st'=>$status, ':c'=>$content, ':grp'=>'why_choose_us', ':tpl'=>'why_choose_us', ':id'=>$id
    ]);
    $pid = $id;
  } else {
    db_execute($pdo, 'INSERT INTO pages (title, slug, content, status, page_group, template_key) VALUES (:t,:s,:c,:st,:grp,:tpl)', [
      ':t'=>$titleIn, ':s'=>$slug, ':c'=>$content, ':st'=>$status, ':grp'=>'why_choose_us', ':tpl'=>'why_choose_us'
    ]);
  ...
  db_execute($pdo, 'DELETE FROM why_page_accordions WHERE page_id = :pid', [':pid' => $pid]);
  ...
  if (is_array($accRows)) {
    ...
    foreach ($accRows as $acc) {
      ...
      db_execute($pdo, 'INSERT INTO why_page_accordions (page_id, title, body_html, is_open, is_active, sort_order) VALUES (:pid,:t,:b,:o,:a,:s)', [
        ':pid' => $pid,
        ':t' => $accTitle,
        ':b' => $accBody,
        ':o' => $accOpen,
  ...
  admin_flash('success', 'Why page saved.');
  header('Location: why-pages.php');
  exit;
}
...
$livePreviewUrl = !empty($pageData['slug']) ? url('why-page.php?slug=' . urlencode((string) $pageData['slug'])) : url('why-page.php');
include __DIR__ . '/_layout_top.php';
?>
<form method="post" class="card card-body" id="whyPageForm" data-section-preview='{"content_type":"page","entity_id":<?= (int) ($pageData['id'] ?? 0) ?>}'>
  ...
  <div class="row g-3">
    ...
    <div class="col-md-6"><label class="form-label">Slug</label><input class="form-control" name="slug" value="<?= e((string)$pageData['slug']) ?>" placeholder="private-label-skin-care-manufacturer"></div>
  ...
  <div class="mt-3"><button class="btn btn-primary">Save Why Page</button> <a href="why-pages.php" class="btn btn-outline-secondary">Back</a></div>
</form>
...
(function () {
  ...
  function ensureEditorId(el) {
    if (!el) return;
    if (!el.id) {
      el.id = 'why_editor_' + (editorCounter++);
    }
  }
  ...
  const form = document.getElementById('whyPageForm');
  if (form) {
