...
$title = 'How It Works — Order Process Accordion';
$pdo = db();
...
if ($pdo && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'save_accordion') {
        ...
        if ($accTitle === '' || $body === '') {
            ...
            header('Location: how-it-works-accordions.php' . ($id > 0 ? '?edit=' . $id : '?action=add'));
            exit;
        }

        // If setting this item as default open, uncheck default open on all other accordions
        if ($isOpenDefault === 1) {
            db_execute($pdo, 'UPDATE how_it_works_accordions SET is_open_default = 0');
        }

        if ($id > 0) {
            $stmt = $pdo->prepare('UPDATE how_it_works_accordions SET title = :title, body = :body, sort_order = :sort_order, is_open_default = :is_open_default, is_active = :is_active WHERE id = :id');
            $stmt->execute([
                ':title' => $accTitle,
                ':body' => $body,
            ...
            $stmt = $pdo->prepare('INSERT INTO how_it_works_accordions (title, body, sort_order, is_open_default, is_active) VALUES (:title, :body, :sort_order, :is_open_default, :is_active)');
            $stmt->execute([
                ':title' => $accTitle,
                ':body' => $body,
                ':sort_order' => $sortOrder,
        ...
        header('Location: how-it-works-accordions.php');
        exit;
    }
    ...
    if ($action === 'toggle_active') {
        $id = (int) ($_POST['id'] ?? 0);
        if ($id > 0) {
            db_execute($pdo, 'UPDATE how_it_works_accordions SET is_active = NOT is_active WHERE id = :id', [':id' => $id]);
            cms_invalidate_how_it_works_cache();
    ...
    if ($action === 'set_default_open') {
        ...
        if ($id > 0) {
            db_execute($pdo, 'UPDATE how_it_works_accordions SET is_open_default = 0');
            db_execute($pdo, 'UPDATE how_it_works_accordions SET is_open_default = 1 WHERE id = :id', [':id' => $id]);
            cms_invalidate_how_it_works_cache();
    ...
    if ($action === 'delete') {
        $id = (int) ($_POST['id'] ?? 0);
        if ($id > 0) {
            db_execute($pdo, 'DELETE FROM how_it_works_accordions WHERE id = :id', [':id' => $id]);
            cms_invalidate_how_it_works_cache();
    ...
    if ($action === 'bulk_delete') {
        ...
        if (!empty($ids)) {
            ...
            $stmt = $pdo->prepare("DELETE FROM how_it_works_accordions WHERE id IN ({$placeholders})");
...
$accordions = $pdo ? cms_get_how_it_works_accordions(true) : [];
...
if ($editId > 0 && $pdo) {
    $row = db_fetch_one($pdo, 'SELECT * FROM how_it_works_accordions WHERE id = :id LIMIT 1', [':id' => $editId]);
    if ($row) {
...
<div class="card shadow-sm border-0 mb-4">
  <div class="card-header bg-white py-3 d-flex justify-content-between align-items-center">
    ...
    <a href="how-it-works-accordions.php" class="btn btn-sm btn-outline-secondary"><i class="bi bi-x-lg"></i> Cancel</a>
  </div>
  <div class="card-body">
    <form method="post" action="how-it-works-accordions.php" data-section-preview='{"content_type":"how_it_works_accordion","entity_id":<?= (int) ($formData['id'] ?? 0) ?>}'>
      ...
      <div class="mt-4 d-flex gap-2">
        ...
        <a href="how-it-works-accordions.php" class="btn btn-outline-secondary">Cancel</a>
      </div>
    </form>
  </div>
</div>
...
<div class="card shadow-sm border-0">
  <div class="card-header bg-white py-3 d-flex flex-wrap justify-content-between align-items-center gap-2">
    <div>
      ...
      <small class="text-muted">Drag rows to reorder steps or edit accordion content.</small>
    </div>
    <a href="how-it-works-accordions.php?action=add" class="btn btn-sm btn-success"><i class="bi bi-plus-lg"></i> Add Accordion Step</a>
  </div>
  <div class="card-body p-0">
    <form id="bulkForm" method="post" action="how-it-works-accordions.php">
      <input type="hidden" name="csrf_token" value="<?= e(csrf_token()) ?>">
      ...
      <div class="p-2 bg-light border-bottom d-flex align-items-center justify-content-between gap-2" id="bulkActionBar" style="display: none !important;">
        ...
        <button type="button" class="btn btn-sm btn-danger" onclick="if(confirm('Delete selected accordion items?')) document.getElementById('bulkForm').submit();">
        ...
        <table class="table table-hover align-middle mb-0" id="accordionsTable">
          <thead class="table-light">
            <tr>
              ...
              <th>Header Title</th>
              <th style="width: 120px;" class="text-center">Default Open</th>
            ...
            <?php else: ?>
              <?php foreach ($accordions as $acc): ?>
                <tr data-id="<?= (int) $acc['id'] ?>" draggable="true" class="draggable-row">
                  ...
                  <td class="text-center">
                    <form method="post" action="how-it-works-accordions.php" class="d-inline">
                      <input type="hidden" name="csrf_token" value="<?= e(csrf_token()) ?>">
                  ...
                  <td class="text-end">
                    <div class="btn-group btn-group-sm me-2">
                      <a href="how-it-works-accordions.php?edit=<?= (int) $acc['id'] ?>" class="btn btn-outline-primary" title="Edit"><i class="bi bi-pencil"></i></a>
                      <form method="post" action="how-it-works-accordions.php" class="d-inline" onsubmit="return confirm('Are you sure you want to delete this step?');">
                        <input type="hidden" name="csrf_token" value="<?= e(csrf_token()) ?>">
